...
そこで本記事では、筆者がRiverpodを好んで使う理由をまとめます。「なぜRiverpodを便利だと思う人がいるのか」を知ることで、Riverpodへの関心を高めてもらえれば幸いです。

## TL;DR

文章が長くなってしまったため、先に結論だけまとめておきます。1万字を超える分量になるとは思っていませんでした……。
以下の4つのポイントは、筆者が感じるRiverpodのメリットです。ただ、実はRiverpodではない手法でも、同じメリットが得られるかもしれません。そういう意味で、Riverpodが特別であると宣伝するつもりはありません。
...
Flutterにおいては、`InheritedWidget`やProviderを利用することで、Flutterのフレームワークが用意した仕組みを利用した`Lifting state up`を実現することができます。`InheritedWidget`やProviderでは、Providerを利用する方が実装しやすく、色々なミスも避けることができるため、現実的にはProviderを採用することになるでしょう。
