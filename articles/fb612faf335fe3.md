...
実装時に意図しないJSライブラリを利用しないように注意したり、flutter_secure_storageに設定するkeyとvalueに気をつけることで、LocalStorageを(おそらく)安全に利用することができるのではと。
...
現時点の理解では、上記2つの点がセキュリティに影響を与える可能性が大きい、と考えています。
なおLocalStorageの振る舞いについては、以下の記事を参考にしています。Flutter Webの開発をしていくために、継続的に勉強せねばと感じているところです。

https://blog.flatt.tech/entry/auth0_access_token_poc

https://mizumotok.hatenablog.jp/entry/2021/08/04/114431

大前提ではあるのですが、ログイン情報などの**本当にセンシティブな情報**は、[firebase_auth](https://pub.dev/packages/firebase_auth)などのサービスを利用するべきです。
