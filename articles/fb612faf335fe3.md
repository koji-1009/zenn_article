...
EncryptedSharedPreferencesを有効にしなかったケースでは、以下の処理が実際に実行されることになります。
(なお、EncryptedSharedPreferencesを有効にすると、ここでコードのチェックは終了となります。)
...
結果として、Android 23以上の端末では、EncryptedSharedPreferencesを利用しないケースであってもAndroid端末のセキュリティ機能を利用し、値を保存することができるようになります。EncryptedSharedPreferences相当の処理を、自前実装しているようなもの、なのではないかなと。

なお筆者は、EncryptedSharedPreferencesとflutter_secure_storageの実装の差異を、細かく比較する能力を持っていません。気になる方は、ぜひお近くのセキュリティエンジニアに、実装の違いなどをご確認いただければと思います。
...
特に有用なのは、以下のIssueです。
(この記事と同じく、EncryptedSharedPreferencesを利用しないケースを確認しています。)
...
flutter_secure_storageのWeb実装は**正しくFlutterアプリが動作していることを前提として**、ある程度は攻撃に対応できるようになっているのでは、と考えています。
実装時に意図しないJSライブラリを利用しないように注意したり、flutter_secure_storageに設定するkeyとvalueに気をつけることで、LocalStorageを(おそらく)安全に利用することができるのではと。とはいえ、ユーザーがdev toolを利用してLocalStorageの中身を確認するケースなど、AndroidやiOSよりも問題がないか検証する必要があるのではないかなと。

---

https://developer.mozilla.org/ja/docs/Web/API/Window/localStorage

LocalStorageは、同一オリジンポリシーを持っており、同一オリジンのJavaScriptからのみアクセス可能です。
またCanvasKitによるレンダリングを利用していれば、HTML DOMの差し替えによる攻撃にも(多少は)強くなっている、と考えています。とはいえ、2023年末現在では、モバイル回線向けにHTMLレンダリングが行われることも多いかもしれません。この意味で、Flutter以外のSPAフレームワークにおける攻撃が、Flutter Webでも問題になる恐れはあると考えています。

また、Flutterの公式ライブラリでは、FlutterやDartのチームが開発していないライブラリに依存しないよう、最新の注意が払われています。
...
問題があるとすれば、アプリの開発のために、開発者が追加するライブラリです。注意が必要なのは、Flutter Web用の実装を確認するのと同時に、ライブラリの中で利用しているJSライブラリも確認する必要がある点です。

---

Flutter Webでは、典型的に`index.html`にJSライブラリのインポートを追加する必要があります。このため、アプリ開発者は`index.html`を確認することで、どのライブラリがどのJSライブラリを読み込んでいるか把握している、と考えているかもしれません。
というのも、Flutter Web向けのライブラリの中には、ライブラリの中で動的にJSライブラリを読み込むものがあるのがあります。これはDartライブラリの利便性を上げてくれるものですが、LocalStorageのセキュリティには影響を与える恐れがあります。

https://github.com/DavBfr/dart_pdf/blob/printing-5.11.1/printing/lib/printing_web.dart#L90-L96

たとえば[pdf](https://pub.dev/packages/pdf)は、Web向けのサポートのためにJSライブラリを実行時に読み込んでいます。
...
大前提ではあるのですが、ログイン情報などの**本当にセンシティブな情報**は、[firebase_auth](https://pub.dev/packages/firebase_auth)などのサービスを利用するべきだと思います。
重ねての強調になりますが、flutter_secure_storageを利用して、Flutter Webでセンシティブな情報を保存する場合には、よくよく検討を行ってください。(よければ、プラクティスとしてFlutter開発コミュニティの中で共有し、よりブラッシュアップしていきたいです。)

## まとめ

flutter_secure_storageは、導入するだけで、データの保存をセキュアに行うことができる素晴らしいライブラリです。そして、AndroidやiOSに馴染みのあるエンジニアであれば、比較的容易に確認することができます。
最近ではLinuxやWindows、macOSなどもサポートされており、ますます便利になっています。複数のプラットフォーム向けに開発を行えるFlutterの利点を活かす意味でも、[shared_preferences](https://pub.dev/packages/shared_preferences)と同等の、欠かせないライブラリの一つだと考えています。
