...
EncryptedSharedPreferencesはAndroid 23以上で利用できる、SharedPreferencesを暗号化して保存するためのクラスです。ただ今回は、**EncryptedSharedPreferencesを利用しない**ケースを主に確認します。
...
`@JS`をつけてクラスを定義することで、Dartの他のクラスから参照したときに、Web Crypto APIなどを呼び出せるようになります。当然ではありますが、型定義を失敗したときには、実行時にエラーとなります。
