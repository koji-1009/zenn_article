...
現在の`paging_view`パッケージには、先頭方向の追加読み込みに課題があります。実装的に読み込む機能は実装できているのですが、読み込んだ際に「読み込み前の位置にスクロール位置を固定したまま、その上に要素を追加する」動作が実現できていません。この動きの実現には、`CustomScrollView`の[center](https://api.flutter.dev/flutter/widgets/ScrollView/center.html)プロパティを活用する必要があります。

Sliverの仕組みを考えると、PrependとAppendのListを中心となるListから分離し、Listの前後に配置することで、スクロール位置を維持したまま要素の追加が可能になると考えています。しかし、この実装には既存のページ保持の方法を改善し、今まで1つのListで完結していたロジックを3つのListに分割する必要があります。今後のバージョンで対応を検討しています。^[このリファクタリングをしたかったので、一度ユニットテストによるカバレッジを100%にしました]
