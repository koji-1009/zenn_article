...
`objective_c`は、DartからObjective-Cのコードを呼び出すためのパッケージです。v9.2.0未満ではbindingコードを`.m`ファイルに記述する必要がありましたが、Hooks(Native Assets)によりDartパッケージに直接Objective-Cコードを組み込めるようになりました。
