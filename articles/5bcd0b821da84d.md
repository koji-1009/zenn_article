...
flutter_hooksは、利用者側からすると「`use`を使っていい感じにhookを呼び出す」ものですが、実装を確認すると「`HookElement`によるWidgetの改変」であることがわかります。`HookElement`を理解すると言えば、そのままflutter_hooksを理解できたと言えるので、`HookElement`を読み解いていきます。

ライブラリのバージョンは、執筆時点の最新バージョンである`0.21.0`を使用します。
...
可読性のためにコードやコメントを省略していくので、一通り把握してから大元のコードに戻って読むことをおすすめします。
...
次の4つのメソッドをオーバーライドしている箇所を、`HookElement`の`build`メソッドに関連する箇所として、最初に見ていきます。
...
ドキュメントを確認する限り、`update`と`reassemble`はHot Reload時に呼ばれるメソッドです。[実装したPR](https://github.com/rrousselGit/flutter_hooks/pull/142)を見てみると、色々と動作を試しながら実装している様子が見て取れます。
`State`ではなく`Element`のAPIを触っているため、普段あまり触れることのないメソッドを使うことになります。

以上の文脈を踏まえ、`HookElement`の`didChangeDependencies`と`build`メソッドを整理したものが、次のコードになります。
...
この2つの条件が変化するのは、`HookState`の[markMayNeedRebuild](https://pub.dev/documentation/flutter_hooks/latest/flutter_hooks/HookState/markMayNeedRebuild.html)を呼び出したときになります。独自のhookを実装する際、利用できるメソッドの1つです。
...
ということで、大半のhookは「`super.build`時に`HookElement._currentHookElement`経由で`HookElement`にアクセスできる」状態になります。
...
flutter_hooksの実装を知っておくと、次のIssueが理解しやすくなります。
